...
int main(void) {
    ...
    if (pid == 0) {
        ...
        // --- Processus parent ---
        do {
            wait(&status);  // Attend que le fils change d'état

            // Vérifie si le fils est terminé ou a été stoppé
            if (WIFEXITED(status) || WIFSIGNALED(status)) {
                puts("child is exiting...");
                return 0;
            }

            // Récupère le syscall en cours dans le registre EAX
